...
from .utils import density_ratio
...
def SCINA(adata, signatures, max_iter=100, convergence_n=10, convergence_rate=0.99,
    ...
    :param allow_unknown: Boolean, default False. If True, allows cells to be assigned to an 'unknown' category.
    ...
    expr = expr.loc[:, [gene for sig in signatures.values() for gene in sig]].copy()
    ...
    for _, markers in signatures.items():
        expr = adata[:, markers].X.toarray()
        
        mean_high = np.percentile(expr, 70, axis=1)
        mean_low = np.percentile(expr, 30, axis=1)
        variances = np.var(expr, axis=1)
        # 此处添加一个小正则项，避免后面出现非正定矩阵而无法对矩阵求逆
        sigma = np.diag(np.maximum(variances, 1e-6))
        print(expr.shape, mean_high.shape, mean_low.shape)
    ...
    while unsatisfied:
        ...
        while iter < max_iter:
            ...
            for r, (cell_type, genes) in enumerate(signatures.items()):
                expr = adata[:, genes].X.toarray()
                print(expr.T.shape, theta[r]['mean'][0].shape, theta[r]['sigma1'].shape)
